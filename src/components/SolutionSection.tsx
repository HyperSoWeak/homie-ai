...
export default function SolutionSection({ t }: SolutionSectionProps) {
  ...
  const originalSchedule = schedules[0].map((item: any) => ({
    ...item,
    tag: undefined,
    oldTime: undefined,
  }));
  ...
  const currentSchedule =
    activeStep === 0 ? schedules[0] : showComparison ? schedules[activeStep] : originalSchedule;
    ...
    if (activeStep === 0) {
      return {
        ...
        reason:
          t.lang === "zh-tw"
            ? "你的完美節奏已恢復，我隨時待命。"
            : "Your perfect rhythm is restored. I'm standing by.",
      };
    }
              ...
              <div className="relative w-[280px] h-[540px] md:w-[340px] md:h-[680px] bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] border-[8px] md:border-[10px] border-slate-800 shadow-2xl overflow-hidden ring-1 ring-white/10 transition-all duration-500 z-10">
                ...
                <div className="absolute inset-0 bg-slate-50 p-6 pt-12 md:p-8 md:pt-14 flex flex-col">
                  <div className="flex justify-between items-center mb-4 md:mb-6">
                    <div>
                      <h4 className="font-bold text-lg md:text-xl text-slate-900">
                        {t.solution.today}
                      </h4>
                    ...
                    <AnimatePresence mode="wait">
                      ...
                      >
                        {showComparison || activeStep === 0
                          ? t.solution.labelAdjusted
                          : t.solution.labelOriginal}
                      </motion.div>
                    </AnimatePresence>
                  </div>
                  ...
                  <div className="flex-1 space-y-3 md:space-y-4 relative overflow-hidden">
                    <AnimatePresence mode="wait">
                      <motion.div
                        key={
                          showComparison ? `step-${activeStep}-after` : `step-${activeStep}-before`
                        }
                              ...
                              >
                                <div className="flex justify-between items-center mb-1">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span
                                      className={clsx(
                                        ...
                                        item.type === "focus" ? "text-slate-400" : "text-slate-500"
                                      )}
                                    >
                                      {item.time}
                                    </span>
                                  ...
                                  <span
                                    className={clsx(
                                      "text-xs md:text-sm font-mono",
                                      item.type === "focus" ? "text-slate-500" : "text-slate-400"
                                    )}
                                  >
                                    {item.duration}
                                  </span>
                                ...
                                <p
                                  className={clsx(
                                    ...
                                    item.type === "focus" ? "text-slate-100" : "text-slate-900"
                                  )}
                                >
                                  {item.task}
                                </p>
